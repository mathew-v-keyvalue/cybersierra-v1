...
The `morpheus` CLI is provided by this plugin and is automatically available on PATH.
...
1. Runs `morpheus auth whoami` — if exit code is non-zero (authentication required):
